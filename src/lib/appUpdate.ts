 ...
 * Play Store as `in.crestly.parents`); iOS falls back to an App Store search.
 *
...
const ANDROID_PACKAGE = "in.crestly.parents";
/** App display name, used for the iOS App Store fallback search. */
const APP_NAME = "Crestly Parents";
...
export async function openAppUpdate(): Promise<void> {
  ...
  // iOS: no numeric App Store id wired yet — open a store search by name.
