/**
 * Crestly Parents — shared UI primitives.
...
export function Screen({
  ...
  // Clear the floating tab bar (~72 tall, sitting at max(insets.bottom,12)),
  // plus a comfortable gap, so the last row always scrolls above it.
  // A real spacer view (not just padding) guarantees this on every device,
  // independent of how the scroll library treats contentContainerStyle.
...
/**
 * Pulsing skeleton block — placeholder while data loads. Use one or two
 * stacked instances in place of spinners on screens with a known layout.
 */
...
/** A consistent ALL-CAPS section title row, with optional trailing action. */
export function SectionHeader({
  ...
  return (
    <View style={styles.sectionHeader}>
      <Text style={styles.sectionHeaderLabel}>{label.toUpperCase()}</Text>
      {action ? (
...
export function EmptyState({
  ...
  const bg = tint?.base ?? colors.white;
  return (
...
const styles = StyleSheet.create({
  ...
  emptyBody: { fontSize: fontSize.bodyS, color: colors.ink60, textAlign: "center", lineHeight: 19 },
  ...
  checkGlyph: { width: 14, height: 14, transform: [{ translateY: -1 }] },
  checkArm1: {
    position: "absolute",
    width: 7,
    height: 2.5,
